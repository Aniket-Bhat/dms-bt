...
QtObject {
    ...
    property bool _autoEnableBluetooth: true
    ...
    property string _activeCategory: ""
            ...
            onRead: data => {
                    ...
                    if (ctrlColonIdx !== -1) {
                        ...
                        if (ctrlProp === "Powered") {
                            ...
                            if (root.btPowered) {
                                ...
                                // once the device list is actually populated
                            } else {
                ...
                if (tag === "NEW") {
                    if (!root.findDevice(mac)) {
                        root.devices.push({
                            mac: mac, name: tail || mac,
                            paired: false, connected: false, available: true,
                ...
                if (line.startsWith("Device ")) {
                    ...
                    if (parts.length >= 2) {
                        ...
                        if (!root.findDevice(mac)) {
                            root.devices.push({
                                mac: mac, name: name,
                                paired: true, connected: false, available: false,
                                icon: "", battery: null, category: "other",
                                connecting: false, error: null
                            })
                        } else {
                            root.findDevice(mac).paired = true
                        }
                    }
                }
            }
        ...
        stdout: SplitParser {
            onRead: data => {
                ...
                if (!root.findDevice(mac)) {
                    root.devices.push({
                        mac: mac, name: name,
                        paired: root.pairedMacs.includes(mac),
                        connected: false, available: true,
                        icon: "", battery: null, category: "other",
                        connecting: false, error: null
                    })
                    if (!root.pairedMacs.includes(mac) && !root._discoveredMacs.includes(mac))
    ...
    property var _batteryRetryTimer: Timer {
        ...
        repeat: false
        onTriggered: {
            if (root._batteryRetryMac !== "") {
                var device = root.findDevice(root._batteryRetryMac)
                if (device && device.battery === null && device.connected)
                    root.fetchDeviceInfo(root._batteryRetryMac)
                root._batteryRetryMac = ""
            }
        }
    }
    ...
    function loadSettings() {
        ...
        _autoEnableBluetooth = pluginService.loadPluginData(pluginId, "autoEnableBluetooth", true)
    }
    ...
    function startPairingMode() {
        if (scanning) return
        scanning = true
        if (_autoEnableBluetooth) powerProc.running = true
        ...
        devices.push({
            mac: "__scanning__",
            name: "Scanning for " + _countdownDuration + "s...",
            paired: false, connected: false, available: false,
            icon: "material:bluetooth_searching", battery: null, category: "other",
            connecting: false, error: null
        })
        refreshItems()
    ...
    function filterDevices(deviceList, query) {
        var pool = deviceList.filter(d => {
            if (d.mac === "__scanning__") return true
            // Show unpaired devices while scanning OR if discovered in this session
            if (!d.paired && (scanning || root._discoveredMacs.includes(d.mac))) return true
            if (_activeCategory && d.category !== _activeCategory) return false
            return true
        })
    ...
    function getCategories() {
        return [
            ...
            { id: "audio", name: "Audio", searchTerm: "audio" },
            { id: "input", name: "Input", searchTerm: "input" },
            { id: "phone", name: "Phone", searchTerm: "phone" }
        ]
    }
